...
/**
 * sfWidgetFormSelectOptGroup represents a select HTML tag.
 *
 * @package    symfony
 * @subpackage widget
 ...
 */
class sfWidgetFormSelectOptGroup extends sfWidgetFormSelect
{
  /**
   * Constructor.
   *
  ...
  protected function getOptionsForSelect($value, $choices)
  {
    ...
    $options = array();
    $i=0;
    echo '1<pre>';print_r($choices);echo '</pre>1';
    foreach ($choices as $key => $option)
    {
      if (is_array($option))
      {
          ...
          foreach($option as $key=>$value){
              echo 'value'.$value;
              if(is_array($value)) {echo 'YES - '.$key.'<br/>';};
          }
