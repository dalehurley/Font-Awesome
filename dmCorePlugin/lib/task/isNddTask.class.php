...
class isNddTask extends lioshiBaseTask {
    ...
    protected function configure() {
        ...
        $this->addOptions(array(
            ...
            new sfCommandOption('verbose', null, sfCommandOption::PARAMETER_REQUIRED, 'quiet mode', 'false')              
        ));
        $this->namespace = 'dm';
        $this->name = 'is-ndd';
...
EOF;
    ...
    protected function execute($arguments = array() , $options = array()) {
        ...
        if (strpos(dmConfig::get('site_ndd'),$options['ndd']) === false ){
            if (!$options['verbose']) $this->logBlock('"'.$options['ndd'].'" is not part the ndd of the site', 'ERROR');            
        } else {
            if (!$options['verbose']) $this->logBlock('"'.$options['ndd'].'" is part of the ndd of the site', 'INFO'); 
