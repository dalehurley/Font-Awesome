...
This is a list of all the values from the <code><b><?php echo realpath($ini_file) ? realpath($ini_file) : $ini_file;?></b></code> INI file.
	...
	*/
	Public Static Function dump($data) {
		...
		//
		/*
                 * Ne sert pas ici car $data est toujours un tableau des arguments passés au helper KrumoHelper, donc un seul argument
                 * if (func_num_args() > 1) {
			$_ = func_get_args();
			foreach($_ as $d) {
				krumo::dump($d);
		...
		//
		?>
<?php $base = sfContext::getInstance()->getRequest()->getRelativeUrlRoot() ?>
<script type="text/javascript" src="<?php echo $base ?>/lfKrumoPlugin/js/krumo.js"></script>
<link rel="stylesheet" type="text/css" media="screen" href="<?php echo $base ?>/lfKrumoPlugin/css/<?php echo sfConfig::get('sf_krumo_dir_skin', 'default/').''; ?>skin.css" />
...
<?php
	...
	*/
	Private Static Function _string($data, $name) {
		...
		$_extra = false;
		$_ = $data;
		if (strLen($data) > sfConfig::get('sf_krumo_string_length')) {
			$_ = substr($data, 0, sfConfig::get('sf_krumo_string_length') - 3) . '...';
			$_extra = true;
			}
?>
<li class="krumo-child">
