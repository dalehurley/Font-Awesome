...
case 2:case 3:
    ...
    var L=new d.walker(K),M,N,O=d.walker.blockBoundary(t==3?{
    br:1
}:null),P=function(R){
    var S=O(R);
    if(!S)M=R;
...
function q(r,s,t){
    ...
    var v=this.getSelection(),w=new d.range(u),x=new h(s=='text'?'textarea':'div',u);
...
j.add('colorbutton',{
    ...
    function q(r,s){
        var t=[],u=m.colorButton_colors.split(','),v=e.addFunction(function(z,A){
            ...
            l.fire('saveSnapshot');
            if(z)D.apply(l.document);else D.remove(l.document);
            l.fire('saveSnapshot');
        });
...
for(u in t)w.elements[u]=q;var x={
    ...
    elements:{$:function(O){var P=O.attributes;if(P){if(P.cke_temp)return false;var Q=['name','href','src'],R;for(var S=0;S<Q.length;S++){R='_cke_saved_'+Q[S];R in P&&delete P[Q[S]];}}return O;},
        embed:function(O){var P=O.parent;if(P&&P.name=='object'){var Q=P.attributes.width,R=P.attributes.height;Q&&(O.attributes.width=Q);R&&(O.attributes.height=R);}},
        param:function(O){O.children=[];O.isEmpty=true;return O;},
        a:function(O){if(!(O.children.length||O.attributes.name||O.attributes._cke_saved_name))return false;},
        body:function(O){delete O.attributes.spellcheck;delete O.attributes.contenteditable;},
        style:function(O){var P=O.children[0];
...
for(u in t)y.elements[u]=r;if(c)x.attributes.style=function(O,P){
...
function w(A){
    return function(B){
        ...
        if(F&&(F.name&&F.name=='br'||F.value&&(G=F.value.match(u)))){
            ...
            else if(H.name=='br')C.splice(D-1,1);else H.value=H.value.replace(u,'');
        }
    };
...
for(y in t.$listItem)z.elements[y]=w(true);j.add('list',{
    init:function(A){
...
j.add('maximize',{
    init:function(q){
        ...
        q.addCommand('maximize',{
            ...
            editorFocus:false,
            exec:function(){
                var z=q.container.getChild(0),A=q.getThemeSpace('contents');
                if(q.mode=='wysiwyg'){
...
(function(){
    j.add('pastefromword',{
        ...
        l.on('paste',function(o){
            var p=o.data,q;
            if((q=p.html)&&(m||/(class=\"?Mso|style=\"[^\"]*\bmso\-|w:WordDocument)/.test(q))){var r=this.loadFilterRules(function(){if(r)l.fire('paste',p);else if(!l.config.pasteFromWordPromptCleanup||m||confirm(l.lang.pastefromword.confirmCleanup))p.html=a.cleanWord(q,l);});r&&o.cancel();}},this);}
            ,loadFilterRules:function(l){var m=a.cleanWord;if(m)l();else{var n=a.getUrl(i.pasteFromWordCleanupFile||this.path+'filter/default.js');a.scriptLoader.load(n,l,null,false,true);}return!m;}});})();(function(){var l={exec:function(o){var p=e.tryThese(function(){var q=window.clipboardData.getData('Text');if(!q)throw 0;return q;},function(){window.netscape.security.PrivilegeManager.enablePrivilege('UniversalXPConnect');var q=window.Components.classes['@mozilla.org/widget/clipboard;1'].getService(window.Components.interfaces.nsIClipboard),r=window.Components.classes['@mozilla.org/widget/transferable;1'].createInstance(window.Components.interfaces.nsITransferable);r.addDataFlavor('text/unicode');q.getData(r,q.kGlobalClipboard);var s={},t={},u;r.getTransferData('text/unicode',s,t);s=s.value.QueryInterface(window.Components.interfaces.nsISupportsString);u=s.data.substring(0,t.value/2);return u;});if(!p){o.openDialog('pastetext');return false;}else o.fire('paste',{text:p});return true;}};function m(o,p){if(c){var q=o.selection;if(q.type=='Control')q.clear();q.createRange().pasteHTML(p);}else o.execCommand('inserthtml',false,p);};j.add('pastetext',{init:function(o){var p='pastetext',q=o.addCommand(p,l);o.ui.addButton('PasteText',{label:o.lang.pasteText.button
    ,command:p});a.dialog.add(p,a.getUrl(this.path+'dialogs/pastetext.js'));if(o.config.forcePasteAsPlainText)o.on('beforeCommandExec',function(r){if(r.data.name=='paste'){o.execCommand('pastetext');r.cancel();}},null,null,0);}
    ,requires:['clipboard']});function n(o,p,q,r){while(q--)j.enterkey[p==2?'enterBr':'enterBlock'](o,p,null,r);};a.editor.prototype.insertText=function(o){this.focus();
            this.fire('saveSnapshot');var p=this.getSelection().getStartElement().hasAscendant('pre',true)?2:this.config.enterMode,q=p==2,r=this.document.$,s=this,t;o=e.htmlEncode(o.replace(/\r\n|\r/g,'\n'));var u=0;o.replace(/\n+/g,function(v,w){
                t=o.substring(u,w);u=w+v.length;t.length&&m(r,t);var x=v.length,y=q?0:Math.floor(x/2),z=q?x:x%2;n(s,p,y);n(s,2,z,q?false:true);
                });t=o.substring(u,o.length);t.length&&m(r,t);this.fire('saveSnapshot');
            };
        
        })();j.add('popup');e.extend(a.editor.prototype,{
            ...
            r.moveTo(p,o);r.resizeTo(m,n);r.focus();r.location.href=l;
            }catch(s){
    ...
    canUndo:false,
    exec:function(n){
            var o,p=n.config,q=p.baseHref?'<base href="'+p.baseHref+'"/>':'',r=b.isCustomDomain();if(p.fullPage)o=n.getData().replace(/<head>/,'$&'+q).replace(/[^>]*(?=<\/title>)/,n.lang.preview);else{
        var s='<body ',t=n.document&&n.document.getBody();if(t){
        if(t.getAttribute('id'))s+='id="'+t.getAttribute('id')+'" ';if(t.getAttribute('class'))s+='class="'+t.getAttribute('class')+'" ';
        }
        s+='>';o=n.config.docType+'<html dir="'+n.config.contentsLangDirection+'">'+'<head>'+q+'<title>'+n.lang.preview+'</title>'+e.buildStyleHtml(n.config.contentsCss)+'</head>'+s+n.getData()+'</body></html>';
        }
        var u=640,v=420,w=80;try{
        var x=window.screen;u=Math.round(x.width*0.8);v=Math.round(x.height*0.7);w=Math.round(x.width*0.1);
        }catch(A){}
        var y='';if(r){
        window._cke_htmlToLoad=o;y='javascript:void( (function(){document.open();document.domain="'+document.domain+'";'+'document.write( window.opener._cke_htmlToLoad );'+'document.close();'+'window.opener._cke_htmlToLoad = null;'+'})() )';
        }
        var z=window.open(y,null,'toolbar=yes,location=no,status=yes,menubar=yes,scrollbars=yes,resizable=yes,width='+u+',height='+v+',left='+w);if(!r){
        z.document.open();z.document.write(o);z.document.close();
        }
    }
    },m='preview';j.add(m,{
        init:function(n){
        n.addCommand(m,l);n.ui.addButton('Preview',{
    ...
    init:function(l){
    var m='print',n=l.addCommand(m,j.print);l.ui.addButton('Print',{
        label:l.lang.print,
    command:m
        });
    }
...
var p=function(){
    var t=this,u=function(){
        ...
        if(a._scaytParams)for(var y in a._scaytParams)x[y]=a._scaytParams[y];var z=new window.scayt(x),A=q.instances[t.name];
        if(A){
            z.sLang=A.sLang;
            z.option(A.option());
            z.paused=A.paused;
...
function q(E){
    ...
    if(J.$.rows.length==1)J.remove();else E.remove();
}
};
...
j.add('tab',{
    ...
    init:function(n){
        ...
        n.addCommand('tab',{
            exec:function(r){
                if(!r.fire('tab'))if(q.length>0)r.insertHtml(q);else return r.execCommand('blur');
                return true;
            }
        });
    ...
    init:function(n){
        ...
        n.on('themeSpace',function(p){
            if(p.data.space==n.config.toolbarLocation){
                n.toolbox=new l();
                var q=['<div class="cke_toolbox"'],r=n.config.toolbarStartupExpanded!==false,s;
...
function o(w){
    if(this.mode=='wysiwyg'){
        this.focus();
        this.fire('saveSnapshot');
        var x=w.data,y=x.getName(),z=f.$block[y],A=this.getSelection(),B=A.getRanges(),C=A.isLocked;
        if(C)A.unlock();
        var D,E,F,G;
...
function u(w){
    var x=w.editor,y=w.data.path,z=y.blockLimit,A=w.data.selection,B=A.getRanges()[0],C=x.document.getBody(),D=x.config.enterMode;
    ...
    if(I&&I.getName&&I.getName() in l){
        p(x);
        if(!c)C.appendBogus();else C.append(x.document.createText('\xa0'));
    }
};
...
j.add('wysiwygarea',{
    ...
    init:function(w){
        ...
        w.on('editingBlockReady',function(){
            var z,A,B,C,D,E,F,G=b.isCustomDomain(),H=function(K){
                if(B)B.remove();
                if(A)A.remove();
                E=0;
                ...
                if(!c)z.append(B);
            },I='<script id="cke_actscrpt" type="text/javascript">window.parent.CKEDITOR._["contentDomReady'+w.name+'"]( window );'+'</script>',J=function(K){
                if(E)return;
                E=1;
...
w.addMode('wysiwyg',{
    ...
    loadData:function(K){
        C=true;
        var L=w.config,M=L.fullPage,N=L.docType,O='<style type="text/css" cke_temp="1">'+w._.styles.join('\n')+'</style>';
    ...
    getData:function(){
        var K=w.config,L=K.fullPage,M=L&&w.docType,N=B.getFrameDocument(),O=L?N.getDocumentElement().getOuterHtml():N.getBody().getHtml();
...
if(b.gecko){
    ...
    (function(){
        ...
        if(!w)v.addEventListener('load',arguments.callee,false);else w.setAttribute('onpageshow',w.getAttribute('onpageshow')+';event.persisted && CKEDITOR.tools.callFunction('+e.addFunction(function(){
        ...
        showBlock:function(o,p,q,r,s){
            ...
            e.setTimeout(function(){
                ...
                var D=e.bind(function(){
                    ...
                    var G=t.element,H=G.getWindow(),I=H.getScrollPosition(),J=H.getViewPaneSize(),K={
    ...
    a.menu=e.createClass({
        ...
        _:{
            showSubMenu:function(m){
                var t=this;
                var n=t._.subMenu,o=t.items[m],p=o.getItems&&o.getItems();
                if(!p){
                    t._.panel.hideChild();
                    return;
                }
...
function R(aa){
    ...
    var ad=(P._.currentFocusIndex+ac+ab.length)%ab.length,ae=ad;
...
this.on('show',function(){
    ...
    if(!ad._.hasFocus){
        ...
        if(ad._.editor.mode=='wysiwyg'&&c){
            var aa=K.document.$.selection,ab=aa.createRange();
...
var W=/cke_dialog_tab(\s|$|_)/,X=/cke_dialog_tab(\s|$)/;
this.parts.tabs.on('click',function(aa){
    var af=this;
    var ab=aa.data.getTarget(),ac=ab,ad,ae;
...
getSize:function(){
    ...
    var K=M._.element.getFirst(),L=M._.size={
...
foreach:function(K){
    var N=this;
    for(var L in N._.contents)for(var M in N._.contents[L])K(N._.contents[L][M]);return N;
},
reset:(function(){
    var K=function(L){
        if(L.reset)L.reset();
...
hide:function(){
    ...
    if(!this._.parentDialog){
        ...
        for(var M in {
            ...
            keydown:1,
            keypress:1
        })K.removeListener(M,J);var N=this._.editor;
        N.focus();
        if(N.mode=='wysiwyg'&&c){
...
addPage:function(K){
    var U=this;
    var L=[],M=K.label?' title="'+e.htmlEncode(K.label)+'"':'',N=K.elements,O=a.dialog._.uiElementBuilders.vbox.build(U,{
        type:'vbox',
        ...
        },L),P=h.createFromHtml(L.join('')),Q=h.createFromHtml(['<a class="cke_dialog_tab"',U._.pageCount>0?' cke_last':'cke_first',M,!!K.hidden?' style="display:none"':'',' id="',K.id+'_',e.getNextNumber(),'" href="javascript:void(0)"',' hidefocus="true">',K.label,'</a>'].join(''));
    if(U._.pageCount===0)U.parts.dialog.addClass('cke_single_page');else U.parts.dialog.removeClass('cke_single_page');
    U._.tabs[K.id]=[Q,P];
    U._.tabIdList.push(K.id);
    U._.pageCount++;
    U._.lastTab=Q;
    var R=U._.contents[K.id]={},S,T=O.getChild();
    while(S=T.shift()){
        R[S.id]=S;
...
function v(K){
    ...
    function R(T){
        ...
        M.y+=Z;
        if(M.x+Q[3]<P)aa=-Q[3];
        else if(M.x-Q[1]>V.width-U.width-P)aa=V.width-U.width+Q[1];else aa=M.x;
        if(M.y+Q[0]<P)ab=-Q[0];
        else if(M.y-Q[2]>V.height-U.height-P)ab=V.height-U.height+Q[2];else ab=M.y;
        K.move(aa,ab);
...
function w(K){
    ...
    function Z(ab){
        ...
        var ai,aj,ak,al;
        if(V.x+P[3]<W)ai=-P[3];
        else if(ah.search('l')!=-1&&V.x2-V.x<M+W)ai=V.x2-M;else ai=V.x;
        if(V.y+P[0]<W)aj=-P[0];
        else if(ah.search('t')!=-1&&V.y2-V.y<N+W)aj=V.y2-N;else aj=V.y;
        if(V.x2-P[1]>ag.width-W)ak=ag.width+P[1];
        else if(ah.search('r')!=-1&&V.x2-V.x<M+W)ak=V.x+M;else ak=V.x2;
        if(V.y2-P[2]>ag.height-W)al=ag.height+P[2];
        else if(ah.search('b')!=-1&&V.y2-V.y<N+W)al=V.y+N;else al=V.y2;
        K.move(ai,aj);
...
(function(){
    k.dialog={
        uiElement:function(K,L,M,N,O,P,Q){
            ...
            var R=(N.call?N(L):N)||'div',S=['<',R,' '],T=(O&&O.call?O(L):O)||{},U=(P&&P.call?P(L):P)||{},V=(Q&&Q.call?Q(K,L):Q)||'',W=this.domId=U.id||e.getNextNumber()+'_uiElement',X=this.id=L.id,Y;
            U.id=W;
            var Z={};
            ...
            var ab=[];
            for(Y in Z)ab.push(Y);U['class']=ab.join(' ');
            if(L.title)U.title=L.title;
            var ac=(L.style||'').split(';');
            for(Y in T)ac.push(Y+':'+T[Y]);if(L.hidden)ac.push('display:none');
            for(Y=ac.length-1;Y>=0;Y--){
                if(ac[Y]==='')ac.splice(Y,1);
            }
            if(ac.length>0)U.style=(U.style?U.style+'; ':'')+ac.join('; ');
            for(Y in U)S.push(Y+'="'+e.htmlEncode(U[Y])+'" ');S.push('>',V,'</',R,'>');
            M.push(S.join(''));
        ...
        hbox:function(K,L,M,N,O){
            ...
            var P=this._.children=L,Q=O&&O.widths||null,R=O&&O.height||null,S={},T,U=function(){
                ...
                for(T=0;T<M.length;T++){
                    var W='cke_dialog_ui_hbox_child',X=[];
...
(function(){
    ...
    regex:function(N,O){
        return function(){
            ...
            if(!N.test(P)){
                if(O!==undefined)alert(O);
                if(Q&&(Q.select||Q.focus))if(Q.select)Q.select();else Q.focus();
                return false;
            }
            return true;
        };
    ...
    return{
        build:function(m,n){
            ...
            var r=m.fire('themeSpace',{
                space:'top',
                html:''
            }).html,s=m.fire('themeSpace',{
                space:'contents',
                html:''
            }).html,t=m.fireOnce('themeSpace',{
                space:'bottom',
                html:''
            }).html,u=s&&m.config.height,v=m.config.tabIndex||m.element.getAttribute('tabindex')||0;
            if(!s)u='auto';
...
a.editor.prototype.resize=function(l,m,n,o){
    var p=/^\d+$/;
    if(p.test(l))l+='px';
    var q=this.container,r=a.document.getById('cke_contents_'+this.name),s=o?q.getChild(0):q;
